...
// don't exit the screen
	...
	UINT8 cy = (player_pos_world[1] + dy) - 0.5; // -1 or +1
	UINT8 tile;
	get_bkg_tiles(cx, cy, 1, 1, &tile);
	return (tile == 0x1B || tile == 0x49 || tile == 0x4A ||
...
void	move_player(INT8 dx, INT8 dy) {
	...
	// move player
		...
		move_sprite(0, player_pos_screen[0], player_pos_screen[1]);
		move_sprite(1, player_pos_screen[0] + 8, player_pos_screen[1]);
		perform_delay(1);
